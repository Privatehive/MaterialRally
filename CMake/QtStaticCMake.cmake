...
macro(qt_generate_qml_plugin_import TARGET)
    ...
    list(APPEND QT_STATIC_QML_DIR "${QT_STATIC_QT_ROOT}/qml")
    ...
    if(QT_STATIC_QML_SRC)
        # Debug
        if(QT_STATIC_VERBOSE)
            message(STATUS "Get Qml Plugin dependencies for ${QT_STATIC_TARGET}. qmlimportscanner path is ${QT_STATIC_QT_ROOT}/bin/qmlimportscanner. RootPath is ${QT_STATIC_QML_SRC} and importPaths is/are ${QT_STATIC_QML_DIR}.")
        endif()

        set(IMPORT_PATH_PARAMS "")
        foreach(DIR IN LISTS QT_STATIC_QML_DIR)
            if (NOT DIR STREQUAL "")
                set(IMPORT_PATH_PARAMS ${IMPORT_PATH_PARAMS} -importPath ${DIR})
            endif()
        endforeach()
        ...
        execute_process(
                COMMAND ${QT_STATIC_QT_ROOT}/bin/qmlimportscanner -rootPath ${QT_STATIC_QML_SRC} ${IMPORT_PATH_PARAMS}
        ...
        foreach(JSON_OBJECT ${QT_STATIC_QML_IMPORT_JSON_OBJ})
            ...
            if(CMAKE_MATCH_1 AND NOT ${CMAKE_MATCH_1} STREQUAL "")
                ...
                if(_PLUGIN_INDEX EQUAL -1)
                    ...
                    message(STATUS "PLUGIN_PATH : ${PLUGIN_PATH} ${_TEMP_CLASSNAME}")
                endif()
            endif()
        endforeach()
        ...
        function(qt_static_generate_path_suffix result parentDirs)
            foreach(parentDir ${parentDirs})
        ...
        foreach(PLUGIN ${QT_STATIC_ALL_PLUGINS})
            ...
            if(${PLUGIN}_PATH)
                find_library(${PLUGIN}_plugin NAMES "${_PLUGIN_LIBRARY_NAME_LOWER}" "${_PLUGIN_LIBRARY_NAME}"
                        HINTS ${${PLUGIN}_PATH})

                find_library(${PLUGIN}_plugind NAMES "${_PLUGIN_LIBRARY_NAME_LOWER}d" "${_PLUGIN_LIBRARY_NAME}d"
                        HINTS ${${PLUGIN}_PATH})
            else()
                find_library(${PLUGIN}_plugin NAMES "${_PLUGIN_LIBRARY_NAME_LOWER}" "lib${_PLUGIN_LIBRARY_NAME_LOWER}"
                        "${_PLUGIN_LIBRARY_NAME}" "lib${_PLUGIN_LIBRARY_NAME}"
                ...
                find_library(${PLUGIN}_plugind NAMES "${_PLUGIN_LIBRARY_NAME_LOWER}d" "lib${_PLUGIN_LIBRARY_NAME_LOWER}d"
                        "${_PLUGIN_LIBRARY_NAME}d" "lib${_PLUGIN_LIBRARY_NAME}d"
                        HINTS ${QT_STATIC_QML_DIR}
                        PATH_SUFFIXES ${PLUGIN_PATH_SUFFIXES})
            endif()
